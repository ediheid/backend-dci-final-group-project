    ...
    if (!errors.isEmpty()) {
      ...
      if (!existingUser) {
        ...
        if (req.body.password === req.body.confirmedPassword) {
          ...
          const response = {
            message:
              "Your new Account has been created! Please check your email to verify your account!",
          };
          ...
          next(
            createErrors(
              406,
              "Your given and confirmed passwords are missmatching. Please make sure you type in the exact same passwords!"
            )
          );
        ...
        next(
          createErrors(
            401,
            "User with the same email already exists. Please make sure you didn't already signup."
          )
        );
      }
    }
    ...
    if (!existingUser) {
      ...
      res.redirect(
        process.env.FRONTEND_URL +
          "/verify-email?message=We were unable to find a user for this verification. Please SignUp!"
      );
      ...
      res.redirect(
        process.env.FRONTEND_URL +
          "/verify-email?message=User has already been verified. Please login!"
      );
    } else {
      ...
      res.redirect(
        process.env.FRONTEND_URL +
          "/verify-email?message=Your account has been successfully activated!"
      );
    ...
    if (!existingUser) {
      ...
      if (validPassword) {
        ...
        try {
          ...
          token = jwt.sign({ userId: existingUser.id }, "XfreshcampingkeyX", {
            expiresIn: "1h",
          });
        } catch {
